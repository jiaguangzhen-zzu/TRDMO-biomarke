...
%          nonZeroElements =  test_adjacency( test_adjacency ~= 0);
%          absMean = mean(abs(nonZeroElements));
%          threshold = absMean * 0.618;
%          test_adjacency(abs(test_adjacency) < threshold) = 0;
        ...
        %for EXP_NUM=1:experiment_num
          ...
          if  fnum == 1 
             ...
             Mask = generate_initial_population(Problem.N,Problem.D, 0.2);
             [Mask,~] = unique(Mask,'rows');
             if size(Mask,1)<Problem.N
                newsoultion=Problem.N-size(Mask,1);
                ns=generate_initial_population(newsoultion,Problem.D, 0.2);
                Mask=[Mask;ns];
             end
...
%                   end 
                   part1 =  Pop; 
                  part3 = generate_initial_population(60,Problem.D, 0.2);
                  
                 random_indices = randperm(size(part1, 1), 60);

% 用 part3 替换 part1 中选中的行
  Mask = part1; % 创建 mask 矩阵
  Mask(random_indices, :) = part3; % 替换行
              if size(Mask,1)<Problem.N
                newsoultion=Problem.N-size(Mask,1);
                  ns=generate_initial_population(newsoultion,Problem.D, 0.2);
                Mask=[Mask;ns];
  ...
  Population_cell = cell(1, 4);
   ...
   Population_cell{row} = concatenated_matrix;
     ...
     for fnum = 1:4
        ...
        filename=strcat('E:\对比试验结果\DNSGA2\Data27\DNSGA2降维',num2str(fnum),'_MMPDENB-RBM_NDSol.mat');
        save(filename,'FVV');
        filename1=strcat('E:\对比试验结果\DNSGA2\Data27\DNSGA2降维',num2str(fnum),'_MMPDENB-RBM_Num.mat');
        save(filename1,'c');
        filename2=strcat('E:\对比试验结果\DNSGA2\Data27\DNSGA2降维',num2str(fnum),'_MMPDENB-RBM_POP.mat');
        save(filename2,'Final_Pop');
        filename3=strcat('E:\对比试验结果\DNSGA2\Data27\DNSGA2降维',num2str(fnum),'_MMPDENB-RBM_boxchart.mat');
        save(filename3,'Non_dominated_sol');
        filename4=strcat('E:\对比试验结果\DNSGA2\Data27\DNSGA2降维',num2str(fnum),'_MMPDENB-RBM_pho.mat');
        save(filename4,'R');
...
function population = generate_initial_population(N, D, p)
    % 生成 N * D 的二进制矩阵
    % N: 个体数量
    % D: 维度
    % p: 每个位置置 1 的概率
